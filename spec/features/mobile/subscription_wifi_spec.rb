...
RSpec.describe "Mobile - Subscription Wifi flow", type: :feature do
  describe "User wants to take a wifi subscription", :headless_mobile do
    ...
    it 'lands on the address step' do
    ...
    it 'is not able to continue without filling the form' do
      click_on 'Continue'
      expect(page).to have_css('.invalid-feedback')
    end

    it 'is not able to continue with a delivery address from another country' do
      if wifi.uk?
    ...
    it 'is not able to continue with a contact phone from another country' do
      if wifi.uk?
    ...
    it 'passes to billing step and save all informations' do
    ...
    it 'redirects to the same subscription from products index' do
      ...
      click_on 'Select offer'
      sleep 1
      expect(page).to have_current_path(edit_subscription_address_path(subscription, address, locale: :en))
    end
  end
end
