...
RSpec.describe CartReflex, type: :reflex do
  ...
  describe 'initializing cart' do
    let(:reflex) { build_reflex(url: new_cart_url(pack: 'starter'), connection: { current_user: user }, params: { pack: 'starter', order: { terms: '1' }, order: { affiliate_link: '' } }) }
  ...
  describe '#init items' do
    context 'with items' do
      let(:reflex) { build_reflex(url: new_cart_url(pack: 'starter'), connection: { current_user: user }, params: { order: { service_ids: [service.id], terms: '1', pack: 'starter', affiliate_link: '' } }) }
    ...
    context 'without items' do
      let(:reflex) { build_reflex(url: new_cart_url(pack: 'starter'), connection: { current_user: user }, params: { order: { terms: '1', pack: 'starter', affiliate_link: '' } }) }
